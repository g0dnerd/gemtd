...
export function mountCombineModal(
  root: HTMLElement,
  game: Game,
  initialTab?: Tab,
): () => void {
  ...
  pickerTitle.textContent = "PICK GEMS";
  ...
  function previewOutput(): {
    ...
    if (activeTab === "level") {
      ...
      const sameQuality = selected.every(
        (t) => t.quality === selected[0].quality,
      );
      if (
        sameGem &&
        sameQuality &&
    ...
    if (combo) {
      if (game.state.phase === "build") {
        const currentCount = selected.filter((t) =>
...
function averageStats(towers: TowerState[]): {
  dmgMin: number;
  dmgMax: number;
  range: number;
  atkSpeed: number;
