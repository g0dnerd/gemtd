...
3. **Gem palette — internal only.** Each `GEM_PALETTE[gem]` entry has both a
   numeric (`light/mid/dark`) and a `.css.{light,mid,dark}` string; the script
   verifies they agree. No cross-file gem check: the HUD reads gem colors via
   `element.style.setProperty('--gem-glow', GEM_PALETTE[gem].css.mid)` (see
   `Hud.ts`), not via static `--gem-*` CSS vars.
...
`['THEME.x', '--px-x']` or `['CELL.x', '--px-x']` line to `MIRROR`.
